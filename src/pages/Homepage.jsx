...
const Homepage = ({ data }) => {
	const [countries, setCountries] = useState(
		localStorage.getItem('countries') ? JSON.parse(localStorage.getItem('countries')) : data
	);
	const [region, setRegion] = useState(localStorage.getItem('region') || 'Filter by Region');
	...
	const filterByQuery = (query) => {
		query = query
			...
			.normalize('NFD')
			.replace(/[\u0300-\u036f]/g, '');
		let filteredData = data.filter((country) => country.name.toLowerCase().includes(query));
